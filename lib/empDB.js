...
class empDB {
    ...
    async findEmployeeById(id) {
        return new Promise((resolve, reject) => {
            connection.query('SELECT employee.first_name, employee.last_name, role.title, role.salary, department.name FROM ((employee' +
                ` INNER JOIN role ON employee.role_id=role.id) INNER JOIN department ON role.department_id=department.id) WHERE employee.id=${id}`, (err, res) => {
                    return err ? reject(err) : resolve(res);
                })
        })
    }
    async searchEmployee({ firstName, lastName }) {
        ...
        if (firstName) {
            query = 'SELECT employee.first_name, employee.last_name, role.title, role.salary, department.name FROM ((employee' +
                ` INNER JOIN role ON employee.role_id=role.id) INNER JOIN department ON role.department_id=department.id) WHERE employee.first_name='${firstName}'`;
            if (lastName) query += ` AND employee.last_name='${lastName}'`
        } else if (lastName) {
            query = 'SELECT employee.first_name, employee.last_name, role.title, role.salary, department.name FROM ((employee' +
                ` INNER JOIN role ON employee.role_id=role.id) INNER JOIN department ON role.department_id=department.id) WHERE employee.last_name='${lastName}'`
        } else return;
        return new Promise((resolve, reject) => {
    ...
    async findManagersByDepartment(departmentId) {
        return new Promise((resolve, reject) => {
            connection.query('SELECT employee.first_name, employee.last_name FROM ((employee' +
                ` INNER JOIN role ON employee.role_id=role.id) INNER JOIN department ON role.department_id=department.id)` +
                ` WHERE department.id=${departmentId} AND employee.is_manager=TRUE`, (err, res) => {
                    return err ? reject(err) : resolve(res);
                })
        });
    }
